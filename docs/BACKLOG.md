...
| T007 | Runner supervisor, named-pipe transport, and GPU scheduler | **Opus** | Sol | T004 | `Optimus.Inference` | All ADR-004 verification tests pass, including the four preemption outcomes at the 20/40/60 ms boundaries, a counter proving no two leases are ever concurrently live, the ASR lease held from capture start to `result`, pre-lease buffering with `LeaseUnavailable` overflow, and the 5-in-5 terminal restart rule |
...
| T018 | Session store, Queue, Steer, questions, and cancellation | **Opus** | Sol | T016 | Core session subsystem | All ADR-005 section 5 and 6 verification tests pass; session metadata survives a Core restart; queue content does not and is reported through the durable `pendingQueueDepth`, delivered once per connecting device, with fault injection in both write windows proving the count never under-reports; `NewSession` coalescing and idempotency behave as specified |
