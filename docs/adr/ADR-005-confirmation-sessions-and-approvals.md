...
- **Write ordering for the recovered count.** On enqueue, the increment is made durable *before* Core sends `SendResult`, so a device is never told an item is queued unless the durable record already counts it. On dequeue, the decrement is made durable *after* the adapter has accepted the send. A crash inside either window therefore leaves the recovered count at most one higher than the number of prompts actually lost, never lower.
- **Startup recovery, before any device is accepted.** Core reads each session's `pendingQueueDepth`, copies non-zero values into an in-memory `queueDroppedCount` for this run, writes `0` back, and flushes. Only then does the listener start. A second crash after that reset but before delivery can lose the notice; v1 accepts this narrow double-crash edge rather than adding a durable per-device notification-ack subsystem. T018 records it as a known limitation and must not describe delivery as guaranteed across repeated startup failures.
...
- T018 and T024 tests assert that `sessions.json` contains no queued prompt text, hash, receipt, or preview at any point, that every queue is empty after a successful restart, and that each affected session reports its `queueDroppedCount` exactly once per connecting device during that Core run. A separate double-startup-crash test records the accepted best-effort notification limitation.
