...
- **Nothing about a prompt is persisted except the opt-in text history and a session title.** Queued prompt content is in-memory and is reported as dropped after a restart, using a durable per-session integer that records how many were pending and nothing about what they said; routine audio is never written at all.
- **Nothing is judged by duration.** A capture that contained no speech is rejected on the runner's voice-activity verdict, so a deliberate one-word command is a first-class prompt and a first-class performance sample.

## The warm path in one paragraph

The user holds the hotkey. The Shell starts an already-initialized WASAPI client and streams 20 ms PCM frames to Core. Accepting the capture clears the GPU slot of speech and background work and takes the ASR lease immediately, so the runner decodes while the key is still held; frames that arrive before the lease is granted are buffered and drained in order. On release, the ASR runner finalizes, Core normalizes and applies the glossary, then the cleanup runner produces a conservative draft that may remove fillers and repair punctuation but may not change intent. Core sends a `PromptDraft`. The user picks an explicit destination and an explicit session, then asks for confirmation; Core issues a `ConfirmationRequest` bound to the exact text, destination, session, device, expiry, and a one-time nonce. The user confirms, the device echoes the exact bytes it rendered from one immutable view model, Core validates and consumes the confirmation, and only then does a sender adapter receive a `ConfirmedPrompt`. The whole path to a visible draft is budgeted at 500 ms p95.
